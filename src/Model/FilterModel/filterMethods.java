...
import Model.PixelModel.Channel;
...
public abstract class filterMethods implements Filter{
    ...
    //HashMap<Integer, Double> filter, ArrayList<Pixel> pixels, int height, int width
    public void linearApplyHelper(HashMap<Integer, Double> filter, Layer layer) {
        ...
        for (int p = 0; p < layer.getPixelList().size(); p++) {
            Pixel currPix = layer.getPixelList().get(p);
