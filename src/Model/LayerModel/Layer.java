...
/**
 * Represents a layer in a multi-layered picture. This will be used by the Project class in order to
 * create a multilayered Project that can exported/imported.
 */
public class Layer extends Export {
  ...
  /**
   * This will allow for a new layer class with a set amount of width and height. Default values are
   * set for other non specified parameters.
   *
  ...
  /**
   * This will allow for a new Layer class with a user specified width, height, and the pixels, so
   * essentially it has a picture already loaded(unless the pixels class is empty which is fine).
   *
   ...
   *                                  such as the pixel size being greater than possible with the
  ...
  public Layer(ArrayList<Pixel> pixels, int width, int height) throws IllegalArgumentException {
    ...
    if (pixels.size() > width * height) {
      throw new IllegalArgumentException(
          "The pixels inputted does not match the width and height specified.");
    }
    this.name = "Untitled Layer";
    this.pixels = pixels;
    this.visible = true;
  ...
  /**
   * This will take in a user-inputted list of pixels, the map of the layer, and the height and
   * width of the layer.
   *
   ...
   * @throws IllegalArgumentException If map or pixels is null, if the width and height are
   *                                  negative, or if the map size does not match with the pixel
   *                                  size
   */
  public Layer(ArrayList<Pixel> pixels, HashMap<Integer, ArrayList<Pixel>> map, int width,
      int height)
    ...
    if (map.size() * width != pixels.size()) {
      throw new IllegalArgumentException(
          "The map inputted is not compatible with the rest of the parameters.");
    }
    this.pixels = pixels;
   ...
   *
   * @return A hashmap mapping each row to an array of pixels. This will make it easier to traverse
   * the pixel list for filterings and such.
   */
  ...
  public void setPixelPositions(HashMap<Integer, ArrayList<Pixel>> newPixelPostions) {
    if (newPixelPostions == null
        || newPixelPostions.size() * this.width > this.width * this.height) {
      throw new IllegalArgumentException("This arraylist is not applicable.");
    }
   ...
   */
  private HashMap<Integer, ArrayList<Pixel>> retPixelPositions(ArrayList<Pixel> pixels, int width,
      int height) {
  ...
  /**
   * Changes whether this is the current layer.
   */
  ...
  public void load(ArrayList<Pixel> newPixels) {
    ...
    if (newPixels.size() != this.width * this.height) {
      throw new IllegalArgumentException("Cannot load a list with the wrong size");
    }
  ...
  public void blend(ArrayList<Layer> imagesToBlend) throws IllegalArgumentException {
    ...
    for (int i = 0; i < this.pixels.size(); i++) {
      ...
      newPixelList.add(new Pixel(
          new Color(newAveragedColor.get(0), newAveragedColor.get(1), newAveragedColor.get(2)),
  ...
  /**
   * s Scrapes the info from the Layer that we will need in order to reload the project later on.
   *
   * @param filePath The file path of the Layer so that we can retrieve it when we/if we are
